...
def _write_kfp_artifacts(run_id: str, model_type: str, config_hash: str, test_metrics: Dict[str, Any], start_time: float, end_time: float, identifier: str) -> None:
    """Write KFP artifact metadata to standard output paths.
    
    Replaces Kafka model-training topic messages with KFP artifacts.
    """
...
def main(df: pd.DataFrame, scaler, experiment_name: str = "NonML", preprocess_meta=None):
    ...
    with mlflow.start_run(run_name=run_name, log_system_metrics=True) as run:
        ...
        if USE_KFP:
            ...
            _jlog("kfp_artifacts_written", run_id=run.info.run_id, model_type=MODEL_TYPE, config_hash=CONFIG_HASH)
        elif USE_KAFKA:
...
# Main execution: KFP mode (default) vs Kafka mode (deprecated)
if USE_KFP:
    # KFP mode: Process training data from artifact (default)
    _jlog("kfp_mode_enabled")
    _process_kfp_training_data()
elif USE_KAFKA:
    # Kafka mode: Start consumer (deprecated)
    _jlog("kafka_mode_enabled_deprecated")
    ...
    _jlog("no_mode_enabled", error="Neither USE_KFP nor USE_KAFKA is enabled")
