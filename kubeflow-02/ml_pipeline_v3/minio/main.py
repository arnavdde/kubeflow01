...
def startup_event():
    ...
    global minio_client
    try:
        # --- Create MinIO client (MinIO Python SDK v7.x syntax) ---
        minio_client = Minio(
            endpoint=ENDPOINT,
        ...
        # --- Upload local 'dataset' directory on startup ---
        ...
        for root, _, files in os.walk(local_dataset_path):
            for file in files:
                ...
                # Preserve directory structure inside MinIO
                object_name = os.path.relpath(local_file_path, local_dataset_path).replace("\\", "/")
                ...
                try:
                    ...
                    logger.info(f"Uploaded '{local_file_path}' as '{object_name}'")
...
async def minio_download(
    ...
    try:
        ...
        file_stream = client.get_object(bucket_name=bucket_name, object_name=object_name)
