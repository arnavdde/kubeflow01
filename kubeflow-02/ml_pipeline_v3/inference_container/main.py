...
USE_BOUNDED_QUEUE = os.environ.get("USE_BOUNDED_QUEUE", "false").lower() in {"1", "true", "yes"} if USE_KAFKA else False
...
def _write_kfp_artifacts(service: Inferencer):
    ...
    # The inferencer stores structured prediction records that would normally go to Kafka
    last_prediction = service.get_last_prediction_copy()
    
    # Build inference_results artifact (JSONL format)
    # In Kafka mode, this would be published to performance-eval topic
    # In KFP mode, we write it to the output artifact
    results_data = []
    if last_prediction:
...
def _process_kfp_inputs(service: Inferencer):
    ...
    try:
        # Read Dataset artifact - expects Parquet file
        ...
        # Store in service for inference
        service.set_df(df)
    ...
    try:
        ...
        # Extract model URI and metadata
        ...
        for cand in uri_candidates:
            try:
                ...
                print({"service": "inference", "event": "kfp_model_loaded", "model_uri": cand, "run_id": run_id})
                loaded = True
                break
...
if __name__ == "__main__":
    ...
    try:
        print({"service": "inference", "event": "main_loop_enter", "mode": "Kafka" if USE_KAFKA else "KFP"})
        
        if USE_KFP:
            # KFP mode: process inputs once and exit (default)
            ...
            # Kafka mode: start runtime and keep process alive (deprecated)
            start_runtime_safe()
            while True:
                time.sleep(1)
